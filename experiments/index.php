<?php
/**
 * NetLogo Web 模型加载器 - 极简版
 * 使用方法：https://tuweb.page.gd/experiments/?id=模型ID
 */
...
function cleanupExpiredFiles() {
    ...
    $lifetime = MODEL_LIFETIME;   // 例如 3600 秒
    $now = time();
    
    // 直接获取所有 .nlogo 文件
    foreach (glob($dir . '*.nlogo') as $filePath) {
        if (file_exists($filePath) && ($now - filemtime($filePath)) > $lifetime) {
            @unlink($filePath);
        }
    }
}
