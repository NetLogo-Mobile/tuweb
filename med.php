...
function loadRemixes() {
    ...
    container.innerHTML = remixes.map(r => {
        ...
        const uid = r.User?.ID || '';
        const avatar = uid && r.User?.Avatar ? `http://netlogo-cn.oss-cn-hongkong.aliyuncs.com/users/avatars/${uid.substring(0,4)}/${uid.substring(4,6)}/${uid.substring(6,8)}/${uid.substring(8,16)}/${r.User.Avatar}.jpg!full` : '<?= DEFAULT_AVATAR ?>';
